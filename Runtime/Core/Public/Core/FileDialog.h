...
class settings {
public:
    static void verbose(bool value) { settings().flags(flag::is_verbose) = value; }

    static void rescan() { settings(true); }
...
protected:
    ...
    explicit settings(bool resync = false) { flags(flag::is_scanned) &= !resync; }
    ...
    inline bool is_kdialog() const { return flags(flag::has_kdialog); }
    ...
    bool &flags(flag in_flag) { return const_cast<bool &>(static_cast<const settings *>(this)->flags(in_flag)); }
};
...
namespace internal {
    ...
    static inline std::string wstr2str(std::wstring const &str) {
        ...
        WideCharToMultiByte(
            CP_UTF8, 0, str.c_str(), (int)str.size(), (LPSTR)ret.data(), (int)ret.size(), nullptr, nullptr);
        return ret;
    }
    ...
    static inline bool is_vista() {
        ...
        DWORDLONG const mask = VerSetConditionMask(
            VerSetConditionMask(
                VerSetConditionMask(0, VER_MAJORVERSION, VER_GREATER_EQUAL), VER_MINORVERSION, VER_GREATER_EQUAL),
    ...
    public:
...
#if __EMSCRIPTEN__
        void start(int exit_code) { m_exit_code = exit_code; }
#endif
        ...
        void start(std::string const &command) {
            ...
            if (!CreateProcessW(
                    nullptr, (LPWSTR)wcommand.c_str(), nullptr, nullptr, FALSE, CREATE_NEW_CONSOLE, nullptr, nullptr,
                    &si, &m_pi))
    ...
    protected:
        ...
        class dll {
        public:
            dll(std::string const &name) : handle(::LoadLibraryA(name.c_str())) {}
        ...
        public:
            ...
            ~new_style_context() { DeactivateActCtx(0, m_cookie); }
    ...
    public:
        bool ready(int timeout = default_wait_timeout) { return m_async->ready(timeout); }
    ...
    protected:
        ...
        std::string osascript_quote(std::string const &str) const {
            return "\"" +
                   std::regex_replace(
                       std::regex_replace(str, std::regex("[\\\\\"]"), "\\$&"), std::regex("'"), "'\\''") +
                   "\"";
        }
    ...
    protected:
        ...
        file_dialog(
            ...
            m_async->start(
                [this, in_type, title, default_path, filter_list, allow_multiselect,
            ...
            if (is_osascript()) {
                ...
                command += " --file-selection --filename=" + shell_quote(default_path) + " --title " +
                           shell_quote(title) + " --separator='\n'";
...
public:
    message(
        std::string const &title, std::string const &text, choice choice = choice::ok_cancel, icon icon = icon::info) {
#if _WIN32
